...
<Steps>
  ...
  <Step title="Student-facing actions halt">
    Choice-filling, document uploads, and payment flows are suspended. Students see a status message like "round temporarily paused".
  </Step>
  <Step title="In-progress actions complete">
    Any action already in progress at pause time - a UPI payment mid-flow
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="What it does not do" icon="unlink">
...
When a control action affects students the platform sends notifications automatically across all active channels.
...
The communication is automatic on control action. The authority does not draft the message instead the console generates it from the action parameters.
