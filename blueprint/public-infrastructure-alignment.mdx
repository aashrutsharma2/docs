...
The Superadmission architecture is designed to align with these existing systems where appropriate. 
...
DigiLocker is a government-operated cloud platform for storage and sharing of official documents. Issued documents from connected issuers are stored in DigiLocker and can be shared with requesters with the document owner's consent.
...
| Seat acceptance fee | ₹10,000 – ₹95,000 | Paid separately per portal |
...
| India Stack Component | Proposed Alignment |
| --- | --- |
| Aadhaar (e-KYC) | Identity layer — optional verification mechanism |
| Aadhaar (Auth) | Student login — biometric or OTP-based |
| DigiLocker (Pull API) | Document layer — fetch verified documents with consent |
| UPI | Payment layer — registration fees, acceptance fees |
| Account Aggregator | Consent architecture reference |
...
**Lower onboarding friction for students.** Familiar infrastructure reduces learning overhead and improves initial adoption.

**Alignment with government digital direction.** India Stack defines components of national digital infrastructure. Systems aligned with it can integrate with existing public infrastructure instead of creating parallel systems.

**Trust through known infrastructure.** Leverages existing identity and document systems, reducing the need to build and maintain parallel verification and storage mechanisms.

<Info>
  The alignment described on this page is a design choice, not a deployment status. 
</Info>
