---
title: "Identity and Verification"
icon: "fingerprint"
description: "The student profile layer. Created once, read by every participating counselling."
mode: "wide"
---

Every inefficiency in India's admissions process traces back to the same gap: no shared student identity across counselling systems. Every portal treats every student as a new, unverified person.

The identity layer closes that gap. One verification. Every counselling.

---

## Verification flow

```mermaid
flowchart LR
    A([Student]) --> B[Aadhaar OTP\nVerification]
    B --> C[UIDAI returns\nName · DOB · Address]
    C --> D[DigiLocker\nConsent]
    D --> E[Documents fetched\nfrom issuing authorities]
    E --> F[APAAR ID\nLinked]
    F --> G([Profile Ready])

    style A fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style B fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style C fill:#D1FAE5,stroke:#34D399,color:#065F46
    style D fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style E fill:#D1FAE5,stroke:#34D399,color:#065F46
    style F fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style G fill:#000000,stroke:#000000,color:#ffffff
```

The student enters their Aadhaar number. Everything after is fetched — not typed.

---

## What the profile contains

<CardGroup cols={2}>
  <Card title="Identity" icon="id-card">
    Name, date of birth, gender, address — pulled from UIDAI. The student types none of this.
  </Card>

  <Card title="Academic records" icon="graduation-cap">
    Class 10 and 12 marksheets fetched from issuing boards via DigiLocker.
  </Card>

  <Card title="Examination scores" icon="chart-line">
    Entrance exam scores fetched from examination authority APIs. Verified at source.
  </Card>

  <Card title="Category documents" icon="file-certificate">
    Caste, income, and domicile certificates fetched from state government via DigiLocker where available.
  </Card>
</CardGroup>

---

## Verification states

| State | What it means | Next step |
| --- | --- | --- |
| `Verified` | Confirmed by issuing authority | Accepted by all integrated counsellings |
| `Manual Review` | Student uploaded; auto-verification unavailable | PraveshAI™ scores confidence — goes to reviewer |
| `Pending` | Fetch in progress | Student notified when complete |
| `Expired` | Document past validity period | Student prompted to refresh |
| `Missing` | Required for a specific counselling | Student alerted before the deadline |

---

## One profile, every counselling

Once verified, the same profile is read by every counselling the student registers for.

```mermaid
graph TD
    P["Verified Student Profile\nAPAAR ID: PV26-DL01-JM78-4K3A"]
    P --> C1[JoSAA\none-tap join]
    P --> C2[JAC Delhi\none-tap join]
    P --> C3[MHT CET\none-tap join]
    P --> C4[WBJEE\none-tap join]

    style P fill:#000000,stroke:#000000,color:#ffffff
    style C1 fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style C2 fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style C3 fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
    style C4 fill:#DBEAFE,stroke:#60A5FA,color:#1E3A8A
```

The counselling authority does not re-run identity verification for students with a verified Superadmission profile. The verification happened once. The result is the input.

---

## Registration time

|  | Current system (per counselling) | Superadmission |
| --- | --- | --- |
| Identity entry | Manual form — 30-45 mins | Aadhaar OTP — 2-3 mins |
| Document upload | Manual PDF per portal — 20-30 min | DigiLocker fetch — automatic |
| Repeat for next counselling | Full process again | Reusable Identity and verified records |
| **Total across 4 counsellings** | **~180 - 240 mins** | **~10 - 15 mins** |

---

## What the student controls

<Steps>
  <Step title="Consent">
    DigiLocker document fetch requires explicit student consent. Granted once, per session, reviewable at any time.
  </Step>
  <Step title="Visibility">
    The student can see every field the platform holds and the verification status per document.
  </Step>
  <Step title="Privacy">
    Profile is private by default. The student can export their data or delete the DigiLocker sync at any time.
  </Step>
  <Step title="Deletion">
    The student can delete their account and revoke all connected identity sync.
  </Step>
</Steps>

<Warning>
  The Aadhaar number is never stored. Only the APAAR ID — a non-reversible derivative — is retained. Payment credentials are never stored. Documents are deleted 90 days after the admission cycle ends.
</Warning>

---

<Info>
  Document Workflows covers what happens after the profile is created — how documents are verified, confidence-scored, and reused.
</Info>